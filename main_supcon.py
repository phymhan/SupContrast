...
def parse_option():
    ...
    opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_negsize_{}_trial_{}'.\
        format(opt.method, opt.dataset, opt.model, opt.learning_rate,
               opt.weight_decay, opt.batch_size, opt.temp, opt.neg_sample_size, opt.trial)
...
def train(train_loader, neg_dataset, model, criterion, optimizer, epoch, opt):
    """one epoch training"""
    model.train()
...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # train for one epoch
        time1 = time.time()
        loss = train(train_loader, neg_dataset, model, criterion, optimizer, epoch, opt)
        time2 = time.time()
